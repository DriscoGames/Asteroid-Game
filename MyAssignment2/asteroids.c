...
void main(){
    int ship_row = FIELD_HEIGHT>>1;     //sets the ship to value 10 midpoint of the screen
    ...
    memset(asteroid_field, 0, sizeof(int)*FIELD_WIDTH*FIELD_HEIGHT);        //sets the memory of a pointer to 0


    initscr();          //sets up the ncurses library with the terminal
    cbreak();           //avoids using buffer, makes pressed keys available instantly to the program without pressing enter
    noecho();           //instead of having pressed key printed to screen, an ncurses function must deal with the key to output it
